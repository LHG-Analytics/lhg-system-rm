...
- **feat(previsão+relatórios+e-mail): previsão ponderada por dia da semana, oportunidades no relatório semanal e envio automático por e-mail** ✅ 2026-07-28 (commit `cb60e30`→próximo; Linear pendente — limite)
  ...
  - Afeta `totalAllValueForecast`, `totalAllRentalsApartmentsForecast`, `totalAllTicketAverageForecast`, `totalAllGiroForecast`, `totalAllTrevparForecast`, **`totalAllRevparForecast`, `totalAllOccupancyRateForecast` e `totalAverageOccupationTimeForecast`** — todos ponderados
  - `queryBigNumbers` passou a expor `totalSuiteDias` (suítes-dia da janela elapsed); `queryTotalRevOcc` passou a expor `rentalRevenue`/`occupiedTime`/`totalSuiteDias` brutos (antes só retornava as taxas derivadas) — necessário para compor RevPAR/Ocupação/TMO previstos com o mesmo método
  - `weekday-forecast.ts`: base histórica por dia da semana agora traz 2 tickets médios em paralelo — `ticketByDow` (valortotal, base do faturamento) e `rentalTicketByDow` (valorliquidolocacao, base do RevPAR) — mais `tmoSecondsByDow` (duração média por locação); `remainingRentalRevenue`/`remainingOccupiedSec` calculados com o mesmo `locacoes_previstas(dow) × métrica_histórica(dow)` já usado pro faturamento
  - RevPAR previsto usa a mesma `forecastSuiteDias` do TRevPAR/Giro (elapsed real + futuro previsto) como denominador — consistência entre os 3 indicadores por suíte-dia
  - TMO previsto = tempo ocupado previsto (elapsed + futuro) ÷ locações previstas; Ocupação prevista = tempo ocupado previsto ÷ (suítes-dia previstas × 86400)
...
- **fix+feat(relatórios): resumo executivo reescrito como briefing de especialista + teste de e-mail manual** ✅ 2026-07-28
  ...
  - **Armadilha grave descoberta ao testar:** o banco do Automo (túnel DDNS residencial, ex: `lushlapa.ddns-intelbras.com.br:25688`) só é confiável a partir da rede da Vercel — rodar `generateWeeklyReport` de uma máquina local dá timeout na maioria das queries paralelas, sobrescrevendo o relatório existente (upsert por `unit_id+period_start`) com dados incompletos. **Nunca regerar relatório de fora do ambiente Vercel** — usar sempre o botão "Gerar agora" da UI (roda como Server Action/Route Handler na Vercel) ou o cron
  - **Limitação desta sessão:** ferramentas de navegador (chrome-devtools/playwright) estavam desconectadas — validação visual feita publicando uma prévia do e-mail (HTML real do template + dados de exemplo) como Artifact, em vez de screenshot direto do app

- **fix(relatorios): redundância no resumo executivo + comparação de dia da semana era trivial** ✅ 2026-07-28
  - **Categoria duplicada:** `opportunities.ts` incluía o nome da categoria dentro do próprio `suggestion` (`"${categoria} — ${label}: ..."`), mas todo lugar que renderiza (e-mail, `opportunities-section.tsx`) já mostra a categoria separada em negrito antes — resultado: "LUSH SPA SPLASH — LUSH SPA SPLASH — sábado...". Fix: `suggestion` não inclui mais a categoria; `opportunitiesBlock` (contexto pra IA) e o fallback de `priorityAction` em `generate-weekly-report.ts` precisaram prefixar a categoria manualmente pra não perder essa informação
  - **Mesma frase repetida 3x:** `priorityAction` (fallback) e a primeira linha de `opportunities` mostravam literalmente o mesmo texto — a IA já usa `opportunities[0]` como base da `priorityAction` por design, então mostrar `opportunities[0]` de novo na lista era redundante. Fix: `ExecutiveSummary` (UI) e `buildEmailHtml` agora pulam o índice 0 na lista secundária (`slice(1, 4)`) — a lista mostra as próximas 3, não repete a nº1. `OpportunitiesSection` (seção ⑦, lista completa) mantém o índice 0 — ali é intencional, é a referência completa
  - **Comparação de dia da semana era trivial:** `detectByDiaSemana` comparava cada dia com a média dos 7 dias juntos — isso sempre aponta "sábado/sexta estão acima da média" pra qualquer categoria, o que é óbvio (fim de semana é mais forte, todo mundo já sabe) e não é uma oportunidade de verdade. Fix: compara cada dia com a média do próprio GRUPO — dias de semana (dom-qui) entre si, FDS (sex-sáb, mesma convenção do `day-band-grid.ts`) entre si — surge desvios reais (ex: quinta anormalmente fraca vs outros dias de semana, sexta muito abaixo de sábado dentro do FDS) em vez de repetir o padrão sazonal já conhecido
  - `send-weekly-report.ts`: `buildEmailHtml` exportado (era função privada) — permite gerar prévias do e-mail via script sem duplicar o template; redesenho visual: grade 2×2 de KPIs com variação vs. semana anterior, caixa de diagnóstico com rótulo "Leitura do período", botão "Resolver com o Agente RM" (`summary.agentPromptLink`) direto no e-mail — antes só existia na página do relatório
  - **Armadilha:** ao adicionar um `dimLabel`/rótulo de dimensão em qualquer render de oportunidade, usar sempre o mesmo mapa (`periodo`→"Período", `turno`→"Turno", `dia_semana`→"Dia") — existem 3 cópias independentes desse mapa (`opportunities-section.tsx`, `send-weekly-report.ts`, fallback em `generate-weekly-report.ts`) por serem 3 contextos de render diferentes (React/e-mail/string simples); manter os 3 sincronizados manualmente

- **feat(relatórios): sazonalidade histórica do período + retry da IA + reforço da comparação a/a** ✅ 2026-07-28
  - **Contexto:** usuário achou o resumo "estático" — mas o texto interpretativo já era gerado por IA (`ANALYSIS_MODEL`); o que ele viu no teste anterior foi o fallback determinístico, acionado só porque a chamada de IA falhou (infra do Automo local, não da IA). Esclarecido; ainda assim havia gaps reais de "inteligência" a fechar
  - **`getSeasonalFactorsForPeriod`** (`src/lib/seasonality/compute.ts`): NOVO — fator sazonal médio (RevPAR/Giro) de um período JÁ FECHADO (a semana do relatório), lendo `unit_seasonality` pelos `date_key` (MM-DD) dos dias do período. Diferente de `getUpcomingSeasonalFactors` (que só olha os próximos 30 dias) — resolve a lacuna de "o resultado dessa semana foi bom por causa da época do ano, ou por um motivo real?"
  - `getSeasonalFactorFor` (função existente) já era um stub que sempre retornava o fator de hoje, ignorando a data pedida — **não corrigido aqui** (fora do escopo desta issue, mas fica registrada como dívida técnica pra LHG-166/decomposição de lift)
  - `buildPastSeasonalityBlock`: bloco markdown "Sazonalidade esperada para este período" injetado no contexto do resumo executivo — instrui a IA a classificar o resultado como dentro do padrão sazonal (sem alarme/comemoração) ou desvio real
  - **Prompt do diagnóstico reforçado:** regra 2 agora EXIGE (não "quando relevante") as duas comparações — semana anterior (momentum) E mesmo período do ano anterior (controla sazonalidade) — e instrui a IA a reconciliar quando contam histórias diferentes; adicionada instrução explícita de que os números do contexto são reais/atualizados direto do ERP, não estimativas — "escreva com confiança"
...
- **fix(critico): middleware bloqueava TODAS as rotas de API — cron parado há ~2 meses** ✅ 2026-08-31
  - **Sintoma:** usuário perguntou "por que não disparou o e-mail que pedi". Investigação: `curl` direto em `/api/cron/revisoes` retornava `307 → /login` em vez do `401` esperado (CRON_SECRET inválido)
  - **Causa raiz:** `src/proxy.ts` (middleware) exigia usuário Supabase autenticado (cookie de sessão) pra qualquer rota não listada como pública — `/api/cron/*` nunca estava na lista. O scheduler de Cron da Vercel nunca tem cookie de sessão (autentica só via `Authorization: Bearer CRON_SECRET`, checado DENTRO do route handler) — toda chamada do cron era redirecionada pro `/login` pelo middleware ANTES do código do CRON_SECRET rodar. `runPendingReviews()` não executava com sucesso desde ~10/07 (confirmado por uma `scheduled_review` travada em `running` desde essa data — o fix de recuperação de travados, já existente, só reseta pra `pending`, nunca chega a rodar de fato)
  - **Fix:** `/api` inteiro passa a ser rota pública pro middleware (`isPublicRoute` inclui `pathname.startsWith('/api')`) — toda rota de API do projeto já faz sua própria autenticação internamente (sessão via `createClient()` ou Bearer `CRON_SECRET`); o middleware nunca deveria interceptar antes disso
  - **2º bug encontrado ao validar o fix:** disparei o cron manualmente (`curl` com `CRON_SECRET` real) pra processar o atraso — 10 revisões de checkpoint processaram com sucesso (notificação `revisao_concluida` criada, relatório gerado), mas TODAS ficaram travadas em `status='running'` mesmo assim. Causa: `scheduled_reviews.conv_id` tem FK pra `rm_conversations`, mas desde LHG-162 (revisões passaram a gerar relatório completo, não conversa) o código salvava ali o id de `rm_weekly_reports` — violando a FK em toda execução bem-sucedida. O `.update()` final falhava silenciosamente (**código não checava o erro retornado**) e a revisão ficava presa em `running` pra sempre, sendo reprocessada (e gerando notificação duplicada) toda vez que o mecanismo de recuperação de travados rodasse
  - **Fix do 2º bug:** migration `20260831000001` adiciona `scheduled_reviews.report_id uuid REFERENCES rm_weekly_reports(id)`; `run-reviews.ts` passa a setar `report_id` (não mais `conv_id`) e agora **checa o erro do update** — se falhar, cai no catch e marca `failed` em vez de ficar mudo. `conv_id` fica intocado (sempre null nesse fluxo). UI (`scheduled-reviews-list.tsx`, botão "Ver análise") trocada de `onSelectConversation(conv_id)` (tentava abrir uma conversa inexistente) pra link direto `/dashboard/agente/relatorios?unit=X`, usando `report_id` como condição de exibição
  - **Correção manual dos dados:** as 10 revisões travadas foram confirmadas (notificação de sucesso correspondente pra cada uma) e marcadas `done` manualmente via script — não dava pra esperar o próximo ciclo, senão seriam reprocessadas de novo (duplicando notificações)
