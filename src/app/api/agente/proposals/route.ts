...
import { generateGiroUpliftRows, buildGiroUpliftBaselineBlock } from '@/lib/pricing/giro-uplift'
...
export async function POST(req: NextRequest) {
  ...
  const [
    ...
    supabase
      ...
      .select('pricing_strategy, max_variation_pct, focus_metric, suite_amenities, unit_goals, budget_yearly, shared_context, pricing_thresholds, pricing_method, giro_uplift_cap, never_reduce')
      .eq('unit_id', unit.id)
      .maybeSingle(),
    supabase
  ...
  const [availabilityRows, realtimeOccupancy, reservationPace, rejectionLessonsBlock, pricingLessonsBlock, seasonalFactors, competitorGaps, elasticityRows, demandPattern, categoryPeriodKPIs, cancellationRates, weeklyPickup] = await Promise.all([
