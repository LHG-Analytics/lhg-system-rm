    ...
    <div className="rounded-xl border bg-card overflow-hidden">
      ...
      {open && (
        <div className="px-5 pb-5 space-y-4">
          ...
          {insights.length === 0 ? (
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-3">
                Comparação dos primeiros 30 dias de cada tabela de preços para medir o impacto real das mudanças.
              </p>
          ...
          {/* Contadores secundários — só mostra se relevante */}
          {(data.newLessonsCount > 0 || data.elasticityUpdatedCount > 0 || data.anomaliesDetected.length > 0 || data.anomaliesResolved.length > 0) && (
            <div className="grid grid-cols-2 gap-3 text-sm">
              {data.newLessonsCount > 0 && <Stat label="Novas lições" value={data.newLessonsCount} />}
              {data.elasticityUpdatedCount > 0 && <Stat label="Elasticidades calculadas" value={data.elasticityUpdatedCount} />}
              {data.anomaliesDetected.length > 0 && <Stat label="Anomalias detectadas" value={data.anomaliesDetected.length} accent="amber" />}
              {data.anomaliesResolved.length > 0 && <Stat label="Anomalias resolvidas" value={data.anomaliesResolved.length} accent="emerald" />}
            </div>
          )}
